# IBPS IT Officer (SO Scale-I) — Prelims Taiyari (Hindi)

> **Aapka Target:** Prelims **pass** (sectional + overall cut-off) | **Time:** 2 mahine | **Role:** IT Officer
> **Rule:** **4 sections** — English, Reasoning, Quant, **Professional Knowledge**. PK = 50 marks (sabse bada)!

---

## 1. Exam Pattern — Prelims (CORRECT ✅ — IT Officer)

| Sr. | Section | Questions | Marks | Medium | Time |
|---|---|---:|---:|---|---|
| 1 | English Language | 25 | 25 | English only | 20 min |
| 2 | Reasoning | 25 | 25 | English & Hindi | 20 min |
| 3 | Quantitative Aptitude | 25 | 25 | English & Hindi | 20 min |
| 4 | **Professional Knowledge** | 25 | **50** | English & Hindi | 20 min |
| | **Total** | **100** | **125** | | **80 min** |

- Har galat answer par **0.25 marks** negative marking.
- Exam **online (CBT)** hota hai.
- **Professional Knowledge** sirf IT Officer (aur kuch specialist posts) ke liye hota hai — yeh section **pass ke liye mandatory** hai.

> ⚠️ Pehle workspace me purana pattern (150 Q, bina PK) tha — **7 July 2026 se** updated plan follow karo.

---

## 2. Sectional Cut-off — SABSE ZAROORI BAAT

IBPS SO Prelims me **har section ka alag cut-off** hota hai:

> Sirf total marks kaafi nahi. **English + Reasoning + Quant + PK** — charo ka minimum clear karna **zaroori** hai.

| Section | Strategy |
|---|---|
| **Professional Knowledge** | 50 marks — **zyada time do**, target 25-35 marks |
| **Reasoning** | 25 Q — puzzles + scoring topics, target 18-22 marks |
| **English** | 25 Q — grammar + vocab, target 12-16 marks |
| **Quant** | 25 Q — sirf easy topics, cut-off clear, target 10-14 marks |

---

## 3. Realistic Target (125 marks total)

| Section | Max | Target | Priority |
|---|---:|---:|---|
| **Professional Knowledge** | 50 | **28 – 38** | 🔴 Highest |
| **Reasoning** | 25 | **18 – 22** | 🔴 High |
| **English** | 25 | **12 – 16** | 🟠 Medium |
| **Quant** | 25 | **10 – 14** | 🟡 Cut-off focus |
| **Total** | 125 | **~70 – 85** | Pass safe zone |

*(Exact cut-off year/category par depend karta hai — PK me strong rehna sabse important.)*

---

## 4. Notes Index

| File | Topic | Priority |
|---|---|---|
| `Study_Plan_8_Weeks.md` | 2 mahine ka plan (PK included) | ⭐ Pehle padho |
| `Professional_Knowledge_Notes_Hindi.md` | PK roadmap + strategy | 🔴 High |
| `Reasoning_Notes_Hindi.md` | Reasoning topics + tricks | 🔴 High |
| `English_Notes_Hindi.md` | Grammar, Vocab, Reading | 🟠 Medium |
| `Quant_Selective_Notes_Hindi.md` | Easy maths (cut-off ke liye) | 🟡 Selective |
| `2026-MM-DD/` folders | Roz ka plan + practice | ⭐ Daily |

---

## 5. Daily Routine (updated — 4 sections)

| Block | Time | Kaam |
|---|---:|---|
| Reasoning | 45 min | Topic + practice |
| English | 35 min | Grammar/vocab/reading |
| Quant | 30 min | Easy topic only |
| **Professional Knowledge** | **45 min** | Theory + MCQ |
| Error Notebook | 15 min | Sabhi sections ki galtiyan |

**Total: ~3 hours** roz.

---

## 6. Mock Practice & Progress Tracking

Daily mock work is kept under dated folders such as `2026-08-20/`. Each folder can contain the daily plan, full mock, mock analysis, and exam JSON used by the practice platform.

### 20 Aug 2026 — Restart Mock ✅

**Mock:** `IBPS IT Officer Prelims — Restart Mock (Comeback Day)`

| Result | Value |
|---|---:|
| Questions | **100** |
| Correct | **99** |
| Wrong | **1** |
| Skipped | **0** |
| Website score | **98.75 / 125** |
| Accuracy | **99%** |
| Time spent | **27m 47s** |

### Section Performance

| Section | Result |
|---|---|
| **Professional Knowledge** | **25 / 25 correct** ✅ |
| **English Language** | **25 / 25 correct** ✅ |
| **Reasoning Ability** | **25 / 25 correct** ✅ |
| **Quantitative Aptitude** | **24 / 25 correct** ⚠️ |

**Only mistake:** Q96 — `25% of 480 = 120`.

> 🔥 **Comeback result: 99/100 correct.** Strong return after the break. Next focus: keep the same accuracy and reduce Quant calculation mistakes.

> ⚠️ The current practice site's result shows **98.75 / 125**. The 20 Aug mock JSON also notes that PK questions are intended to carry 2 marks each, so the scoring implementation should be reviewed separately.

---

## 7. Exam Day Order (suggested)

1. **Professional Knowledge (20 min)** — fresh mind, 2 marks per Q
2. **English (20 min)** — quick scoring
3. **Reasoning (20 min)** — puzzles need focus
4. **Quant (20 min)** — sirf easy/known questions

> **Golden Rule:** Negative marking (-0.25). Guess kam karo. PK me bhi confident ho tabhi answer do.

---

> Shubhkamnayein! 💪 IT Officer ke liye PK = half the battle. Roz padho, mock me charo sections lo.
